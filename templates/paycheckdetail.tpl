...
<p>Taxes:</p>
<ul>
	<li><strong>FEDERAL:</strong> {% if federal_tax %}${{ federal_tax.amount|floatformat:"2" }}  <form class="delete" action="" method="post"><input type="hidden" name="d-expense-key" value="{{ federal_tax.key.name }}" /><button type="submit">Delete</button></form>{% else %}<em>Not entered</em>{% endif %}
  <li><strong>STATE:</strong> {% if state_tax %}${{ state_tax.amount|floatformat:"2" }}  <form class="delete" action="" method="post"><input type="hidden" name="d-expense-key" value="{{ state_tax.key.name }}" /><button type="submit">Delete</button></form>{% else %}<em>Not entered</em>{% endif %}
  {% if other_taxes %}
  {% for tax in other_taxes %}
	<li>{{ tax.name }}: ${{ tax.amount|floatformat:"2"}}  <form class="delete" action="" method="post"><input type="hidden" name="d-expense-key" value="{{ tax.key.name }}" /><button type="submit">Delete</button></form></li>
	{% endfor %}
	{% else %}
...
<p>Deductions:</p>
<ul>
	<li><strong>MEDICAL:</strong> {% if med_insurance %}${{ med_insurance.amount|floatformat:"2" }}  <form class="delete" action="" method="post"><input type="hidden" name="d-expense-key" value="{{ med_insurance.key.name }}" /><button type="submit">Delete</button></form>{% else %}<em>Not entered</em>{% endif %}
	<li><strong>DENTAL:</strong> {% if dental_insurance %}${{ dental_insurance.amount|floatformat:"2" }}  <form class="delete" action="" method="post"><input type="hidden" name="d-expense-key" value="{{ dental_insurance.key.name }}" /><button type="submit">Delete</button></form>{% else %}<em>Not entered</em>{% endif %}
	<li><strong>LIFE:</strong> {% if life_insurance %}${{ life_insurance.amount|floatformat:"2" }}  <form class="delete" action="" method="post"><input type="hidden" name="d-expense-key" value="{{ life_insurance.key.name }}" /><button type="submit">Delete</button></form>{% else %}<em>Not entered</em>{% endif %}
	<li><strong>VISION:</strong> {% if vision_insurance %}${{ vision_insurance.amount|floatformat:"2" }}  <form class="delete" action="" method="post"><input type="hidden" name="d-expense-key" value="{{ vision_insurance.key.name }}" /><button type="submit">Delete</button></form>{% else %}<em>Not entered</em>{% endif %}
	<li><strong>401k:</strong> {% if 401k %}${{ 401k.amount|floatformat:"2" }}{% else %}<em>Not entered</em>{% endif %}
  {% if deductions %}
  {% for d in deductions %}
	<li>{{ d.name }}: ${{ d.amount|floatformat:"2"}} <form class="delete" action="" method="post"><input type="hidden" name="d-expense-key" value="{{ d.key.name }}" /><button type="submit">Delete</button></form></li>
	{% endfor %}
	{% else %}
...
<ul>
	<li>Food &amp; Dining: ${{ food_expenses_total|floatformat:"2" }}
		<ul>
			...
			<li>{{ expense.date }} - {{ expense.vendor }}: ${{ expense.amount|floatformat:"2" }} <form class="delete" action="" method="post"><input type="hidden" name="d-expense-key" value="{{ expense.key.name }}" /><button type="submit">Delete</button></form></li>
			{% empty %}
